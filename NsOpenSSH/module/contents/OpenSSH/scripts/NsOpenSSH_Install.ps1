function Get-NsOpenSSH {
	...
	begin {
		...
		# VARIABLES
			...
			New-Variable -Name 'scriptStage' -Value "$HOME\.stage" -Scope 'Local' -ErrorAction 'Stop' -Description "This hidden, temporary folder is (as the name suggests) is meant to provide a space in which files and directories can be organized, modified, etc. As the script completes, the folder will be removed- so if you (if 'you' are the user) find it, just delete it." -WarningVariable +NsWarningVar -ErrorVariable +NsErrorVar -InformationVariable +NsInfoVar -PassThru
